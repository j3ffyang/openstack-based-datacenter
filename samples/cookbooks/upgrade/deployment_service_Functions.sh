...
installServiceServer() {
	...
	./deploy_deployment_service.sh -p $OS_PWD < /dev/null > ${INSTALL_LOG}/deploy_deployment_service.log 2>&1
	DEPLOY_RESULT=$?
	popd
...
setServiceServerConfig() {
	...
	then
        ...
        sed -i  's#10.0.0.0/24#9.115.78.0/24#' ${DEPLOYMENT_CONFIG}
        sed -i  's#10.0.0.100#9.115.78.80#' ${DEPLOYMENT_CONFIG}
        sed -i  's#10.0.0.200#9.115.78.118#' ${DEPLOYMENT_CONFIG}
        sed -i  's#NETWORK_MANAGER#nova.network.manager.FlatDHCPManager#' ${SCO_INSTALL_DIR}/data/openstack/metadata.json
