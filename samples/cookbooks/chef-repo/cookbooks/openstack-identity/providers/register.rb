...
action :create_endpoint do
  if node['openstack']['identity']['catalog']['backend'] == 'templated'
    ...
    begin
      ...
      if endpoint_uuid
        Chef::Log.info("Endpoint already exists for Service Type '#{new_resource.service_type}' already exists.. clean and recreated.")
        identity_command(new_resource, 'endpoint-delete',
                         '' => endpoint_uuid)
        Chef::Log.info("delete endpoint for service type '#{new_resource.service_type}'")
      end
