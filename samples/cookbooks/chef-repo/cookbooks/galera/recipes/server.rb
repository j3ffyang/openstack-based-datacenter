...
galera_config = data_bag_item(node['galera_config']['data_bag_name'], node['galera_config']['data_bag_item_name'])
...
bash "set-wsrep-grants-mysqldump" do
  user "root"
  code <<-EOH
    #{node['mysql']['mysql_bin']} -uroot -h127.0.0.1 -e "GRANT ALL ON *.* TO '#{node['wsrep']['user']}'@'%' IDENTIFIED BY '#{node['wsrep']['password']}'; UPDATE mysql.user SET grant_priv='Y' where user='root'; FLUSH PRIVILEGES;"
  ...
  EOH
  only_if { my_ip == init_host && (galera_config['sst_method'] == 'mysqldump' || galera_config['sst_method'] == 'rsync' ) && !FileTest.exists?("#{install_flag}") }
end
